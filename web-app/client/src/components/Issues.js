...
import { MDBCardBody, MDBInput, MDBCol, MDBRow, MDBCardText } from "mdbreact";
...
const API_URL = "http://localhost:6500/request/";
...
const Issues = () => {
  ...
  const HandleProductInfo = async () => {
    ...
    if (Identifier) {
      await axios
        .post(API_URL + "viewProduct", { identifier: Identifier })
        .then((response) => {
  ...
  const HandleIssueReport = async () => {
    ...
    if (Identifier) {
      ...
      await axios
        .post(API_URL + "updateMessage", IssueData, {
        ...
        .then((response) => {
          ...
          if (response.status === 200 && response.data.message === "success") {
            setLoad(false);
            setIssueBox(false);
            setMessage("Safety Message Recorded");
            setIssue("");
  ...
  return (
    <>
      <Modal
        size="lg"
        show={issueBox}
        onHide={() => setIssueBox(false) && setIssue("")}
        aria-labelledby="example-modal-sizes-title-lg"
      >
        <Modal.Header closeButton>
          <Modal.Title id="example-modal-sizes-title-lg">
            Reporting a Safety Message for {Identifier}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <>
            {data ? (
              <>
                {data.safety_message.message === "" ? (
                  <>
                    <Alert severity="info">NO reported issues.</Alert>
                  </>
                ) : null}
                {data.safety_message.message !== "" ? (
                  <>
            ...
            <MDBInput
              label="Enter the Safety Message"
              type="text"
              success="right"
        ...
        <Modal.Body>
          ...
          <MDBRow>
            ...
            <MDBCol>
              {data ? (
                <>
                  <div style={{ textAlign: "center" }}>Safety Messages</div>
                  <br />
                  {data.safety_message.message === "" ? (
                    <>
                      <Alert severity="info">NO reported issues.</Alert>
                  ...
                  <div style={{ textAlign: "center" }}>
                    Product Certifications and Inspection Information
                  </div>
                  <br />
                  {data.certification_and_inspection_info
                  ...
                  <br />
                  {data.certification_and_inspection_info.third_party_inspection
                    .type !== "" ? (
                    <>
                      <Alert variant="filled" severity="info">
                        ...
                        {
                          data.certification_and_inspection_info
                            .third_party_inspection.type
                        }{" "}
                        :{" "}
                        {
                          data.certification_and_inspection_info
                            .third_party_inspection.result
                        }
                      </Alert>
                    </>
                  ) : (
      ...
      <MDBCardBody>
        ...
        <div style={{ width: "80%", textAlign: "center", margin: "auto" }}>
          <MDBCardText>Please enter the product identification</MDBCardText>
          <MDBInput
            ...
            value={Identifier}
          />
          <div>{error ? <Alert severity="warning">{error}</Alert> : ""}</div>
          <div>{message ? <Alert severity="info">{message}</Alert> : ""}</div>
          <Button
            type="button"
            gradient="blue"
